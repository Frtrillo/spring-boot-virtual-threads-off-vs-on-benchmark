...
echo "=== JavaScript IoT Benchmark (Fastify) ==="
...
if [ "$RUNTIME" = "bun" ]; then
    RESULT_FILE="result_bun.txt"
    RUNTIME_DISPLAY="Bun"
else
    RESULT_FILE="result_fastify.txt"
    RUNTIME_DISPLAY="Fastify (Node.js)"
fi
...
if [ -f "result_bun.txt" ]; then
    echo "Fastify (Bun):"
    grep -E "(Requests/sec|Transfer/sec)" result_bun.txt || echo "No results found"
    echo ""
fi

echo "To run other benchmarks:"
echo "  Spring Boot: ./run_benchmark_fixed.sh"
echo "  Express (Node.js): ./run_nodejs_benchmark.sh"
